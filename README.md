...
- **自動循環**：每 14 天自動清空已推播紀錄，無需登入 Render 手動重置。
...
├── seen_repo_reset_at.txt     # 自動重置時間（不納入 Git）
...
Bot 每輪檢查時會確認距離上次重置是否已超過 14 天。達到週期後，程式會清空 `seen_repo_ids`、保存新的重置時間，下一輪即可重新推播熱門專案。重置時間保存在 `seen_repo_reset_at.txt`，因此 Bot 重新啟動後也能延續週期計算。
...
- `seen_repo_reset_at.txt`：由程式自動管理，不需要手動編輯或提交到 Git。
